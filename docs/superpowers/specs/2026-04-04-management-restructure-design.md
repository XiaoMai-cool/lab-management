...
| 直接报销 | 无需入库 | 流程 | 典型场景 |
|---------|---------|------|---------|
| ✗ | ✗ | 审批 → 报销 → 入库 | 学生采购药品/设备（默认） |
| ✗ | ✓ | 审批 → 报销 | 需审批但无实物入库 |
| ✓ | ✗ | 报销 → 入库 | 已购物资补报销 |
| ✓ | ✓ | 仅报销 | 差旅、快递等 |

- 学生/老师提交采购申请（标题、金额、类别、采购类型、描述、附件）
- 未勾选「直接报销」时：教师审批通过后，记录进入「已批准」状态
- 勾选「直接报销」时：跳过审批，直接进入待报销状态
...
- 老师（教师角色）提交采购申请时，**自动跳过审批环节**（系统自动勾选「直接报销」，老师无需手动操作），记录直接进入待报销状态
- 报销流程正常走李健楠审批，与学生一致
...
- **耗材专人（王子寒）/ 药品专人（宋艳芳）**：看到需要入库的记录（`skip_registration = false`，按类别自动路由：药品→宋艳芳，非药品→王子寒），登记时可看到该记录的报销状态（已报销/未报销）
...
purchases:
  ...
  approval_note       -- 审批备注
  approved_at         -- 审批时间
  auto_approved       -- boolean, 老师自行采购或勾选「直接报销」时为 true
  skip_registration   -- boolean, 勾选「无需入库」时为 true
...
页面展示审批通过的采购记录，管理员可标记「已登记」，同时可看到该记录的报销状态（已报销/未报销）。
...
| 当前页面 | 变更 |
|---------|------|
| `PurchaseApprovalForm.tsx` | 改为统一的采购申请表单。老师提交时自动跳过审批。默认选中对应教师，可手动切换 |
| `PurchaseApprovalList.tsx` | 改为学生/老师的采购记录列表，显示完整流程线（审批→报销）。区分学生和老师的记录 |
| `PurchaseApprovalReview.tsx` | 保留教师审批功能，增加已审批记录的后续状态展示（报销状态），区分学生/老师申请 |
| `ReimbursementForm.tsx` | 改为在已批准的采购记录上补充报销信息 |
| `ReimbursementList.tsx` | 废弃，合并到采购记录列表 |
| `ReimbursementReview.tsx` | 保留，报销审批人审批报销 |
| `ReimbursementStats.tsx` | 增加 Tab 切换（全部/非药品/药品）+ 增强筛选（姓名、时间范围、金额范围） |
