...
- 报销流程正常走李健楠审批，与学生一致
- 老师的采购记录在审批列表中与学生记录区分显示

#### 学生选择审批教师

- 学生提交采购申请时需选择审批教师
- 系统根据学生-教师对应关系**自动默认选中**对应的老师
- 学生也可以手动切换为其他老师（保留下拉菜单可选）
...
新增入库登记功能，按类别分别归入对应管理分组：

- **非药品入库** → 放在「耗材管理」分组，耗材专人可见
- **药品入库** → 放在「药品管理」分组，药品专人可见

页面展示审批通过且 `skip_registration = false` 的采购记录，管理员可标记「已登记」，同时可看到该记录的报销状态（已报销/未报销）。
